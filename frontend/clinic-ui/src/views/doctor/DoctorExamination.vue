...
medicineId: m.id, // Gán ID ở đây nữa
...
// Đổ dữ liệu cũ vào Form để bác sĩ Cập nhật
...
// 2. Chuẩn bị danh sách xét nghiệm
const clinicalTestNames = form.requestClinicalTest
? form.clinicalTests.filter(t => t && t.trim()).map(t => t.trim())
: []
// 3. Chuẩn bị đơn thuốc - QUAN TRỌNG: Gửi medicineId và mapping lại số lượng
const prescriptionItems = form.prescriptionItems
.filter(item => (item.medicineName && item.medicineName.trim()) || item.medicineId)
.map(item => ({
medicineId: item.medicineId, // Gửi ID để database không bị NULL
medicineName: item.medicineName,
dosage: item.dosage,
// Backend Nhàn đang dùng TotalQuantity và UnitPrice,
// nên ta gửi đúng field hoặc gửi quantity để Backend xử lý
totalQuantity: Number(item.quantity) || 1,
duration: Number(item.quantity) || 1, // Vì Nhàn nói duration cũng là số lượng
quantity: Number(item.quantity) || 1
}))
// 4. Gọi API Lưu/Cập nhật
// Trong hàm submit() của file Vue
...
clinicalTestNames,
// Sửa tên key này cho khớp với DTO ở Backend (thường là Details)
prescriptionItems: prescriptionItems, // Đổi details thành prescriptionItems cho khớp DTO insuranceCoverPercent: dataFromRecord.value?.insuranceCoverPercent ?? 0,
